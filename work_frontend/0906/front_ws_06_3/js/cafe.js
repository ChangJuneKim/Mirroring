...
window.onload = function () {
  ...
  if (poll) {
    ...
    pollContent += '<div class="fw-bold">' + question + "</div>";
    ...
    pollContent += "</div>";
    pollContent +=
      '<div class="text-end text-secondary mt-1">투표기간 : ' +
      dateFormat(sdate) +
      " ~ " +
      dateFormat(edate) +
      "</div>";
...
function programmingBook(response) {
...
function addAnswer() {
  ...
  divEl.setAttribute("class", "poll_answer_item input-group mb-1"); // <div class="poll_answer_item"></div>
  var inputEl = document.createElement("input"); // <input/>
  inputEl.setAttribute("type", "text"); // <input type="text"/>
  inputEl.setAttribute("class", "form-control me-3"); // <input type="text" class="form-control"/>
...
function makePoll() {
  ...
  document.querySelector("#voteModal").modal("hide"); // 모달창 닫기.
}
