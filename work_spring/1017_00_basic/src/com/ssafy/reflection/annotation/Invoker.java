 ...
 * 자바의 리플렉션(Reflection)
 * 
 * 리플렉션은 자바에서 기본적으로 제공하는 API 클래스, 인터페이스 ,메서드들을 찾을 수 있고 객체를 생성하거나 변수를 변경할 수 있고
 * 메서드를 호출할 수도 있다
 * 
 * 리플렉션은 Class Constructor Method Field 와 같은 정보를 가져올 수 있다
 * 
 * 이 정보들을 가져와서 객체를 생성하거나 메서드를 호출하는 변수의 값을 변경 가능하다
 */
public class Invoker {

	// obj 내부에서 생성한 객체를 전달 받음
	...
	public void invoke(Object obj) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException {
		...
		// 3. Order 어노테이션의 number값을 확인하여 순서대로 메서드 저장할 배열
		Method[] orders = new Method[methods.length];

		// 4. Order 어노테이션 정보를 바탕으로 orders 배열에 메서드 저장
