 ...
 * 트랜잭션이나 보안, 시간측정 등이 Aspect의 좋은 예 이다.
 * 
 * Advice
 * 언제 공통 관심 기능을 핵심 로직에 적용할지를 정의하고 있다.
 * 예를 들어, '메서드를 호출하기 전'(언제) 에 '성능측정을 시작한다'(공통기능) 기능을
 * 적용한다는 것을 적의하고 있다.
 * 
 * Joinpoint
 * Advice 를 적용 가능한 지점을 의미한다. 메서드 호출, 필드 값 변경 등이
 * Jointpoint에 해당한다.
 * 
 * Pointcut
...
public @interface Aspect {
	String target(); // 어느 핵심 로직에 Aspect를 적용할지
	String timing(); // 어느 시점에 Aspect를 실행할지
}
