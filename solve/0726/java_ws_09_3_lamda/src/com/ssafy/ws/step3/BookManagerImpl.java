...
public class BookManagerImpl implements IBookManager {
	...
	static private IBookManager instance;
	...
	public Magazine[] getMagazines() {
		int count = 0;
		for (int i = 0; i < size; i++) {  // books 배열에서 잡지 개수만큼 카운트
	...
	public Book[] getBooks() {
		int count = 0;
		for (int i = 0; i < size; i++) {  // books 배열에서 일반 책 개수만큼 카운트
			if (!(books[i] instanceof Magazine)) {
				count++;
			}
		}
		...
		int idx = 0;
		for (int i = 0; i < size; i++) {
			if (!(books[i] instanceof Magazine)) {  // 일반 책만 배열에 담기
				result[idx++] = books[i];
			}
		}
