...
public class HomeDaoImpl implements HomeDao {
	...
	private List<EnvironmentInfo> envMapList;
	...
	public void loadData() {
		...
		envMapList = csv.getEnvironmentInfos();
	}
	...
	public HomeDeal search(int no) {
		...
		for (HomeDeal homeDeal : search) {
			if (homeDeal.getNo() == no) {
				String img;
				String str = homeDeal.getDong() + homeDeal.getAptName();
				if (homeInfo.get(str) != null) {
					img = homeInfo.get(str).getImg();
				} else {
					img = null;
				}
				homeDeal.setImg(img);
	...
	public List<EnvironmentInfo> searchEnvInfo(String name) {
		envMapList.clear();
		CSVHandler csv = new CSVHandler();
		envMapList = csv.getEnvironmentInfos();
		List<EnvironmentInfo> res = new ArrayList<>();
		for (EnvironmentInfo it : envMapList) {
			if (it.getAddr().contains(name)) {
				res.add(it);
			}
		}
		if (res.size() != 0) {
