...
/* 여기서 부터 풀이 시작입니다. */
#1 classicmodels 데이터베이스를 사용하도록 설정하시오.
use classicmodels;

#2 orders 테이블에서 orderNumber가 10100인 고객의 Number(customerNumber)를 조회하시오.
select customerNumber
from orders
where orderNumber = 10100;

#3 orders 테이블에서 customerNumber가 100이상 150이하인 주문건을 조회하시오. (90건)
select *
from orders
where customerNumber between 100 and 150;

#4 orders 테이블에서 주문의 배송 상태가 몇가지가 되는지 조회하시오.
select distinct status
from orders;

#5 orderdetails 테이블에서 상품 가격이 높은 순으로 5개의 데이터만 출력하시오.
select *
from orderdetails
order by priceEach desc
limit  5;

#6 employees 테이블에서 lastName에 B가 들어가는 직원을 모두 조회하시오.
select *
from employees
where lastName like '%B%';

#7 employees 테이블에서 employeeNumber가 1500이하이고 officeCode가 4인 직원은 lastName과 firstName을 합쳐 Full Name(as 이름)과
# employeeNumber, jobTitle을 조회하시오
select concat(lastName, ' ', firstName) as 이름, employeeNumber, jobTitle
from employees
where employeeNumber <= 1500 and officeCode = 4;

#8 payments 테이블에서 가장 최근에 결제된 건의 checkNumber를 뒤집어서 출력하시오.
select reverse(checkNumber)
from payments
order by paymentDate desc
limit 1;

#9 offices 테이블에서 Boston에 위치한 사무실 우편번호와 주소(상세주소까지), 전화번호를 출력하시오.
select postalCode as '우편번호', concat(addressLine1, ' ', addressLine2) as 주소, phone as '전화번호'
from offices
where city = 'Boston';

#10 products 테이블에서 productline이 motorcycles인 제품을 재고가 많이 남은 순으로 조회하시오.
select *
from products
