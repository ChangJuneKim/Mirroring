...
public class ProfessorSolution {
	...
	private static int W;  // 맵의 가로길이
	private static int H;  // 맵의 세로길이
		...
		while (!queue.isEmpty()) {
			...
			while (--size >= 0) {
				...
				for (int i = 0; i < 4; i++) {
					...
					// 경계 안이고 방문하지 않은 곳이면
					if ((0 <= testX && testX < H) && (0 <= testY && testY < W) && !isVisited[testX][testY][curMonkey.k]) {
				...
				if (curMonkey.k > 0) {  // 말처럼 뛸 수 있는 횟수가 있다면
					...
					for (int i = 0; i < 8; i++) {
						...
						// 경계 안이고 방문하지 않은 곳이면
						if ((0 <= testX && testX < H) && (0 <= testY && testY < W) && !isVisited[testX][testY][curMonkey.k - 1]) {
							
							// 평지일 경우
							if (map[testX][testY] == GROUND) {
								
								// 방문 처리
								isVisited[testX][testY][curMonkey.k - 1] = true;
