...
public class ProfessorSolution {
	...
	private static int[][] magnets;  // 자성정보
	...
	public static void main(String[] args) throws Exception {
		...
		for (int test_case = 1; test_case <= T; test_case++) {
			...
			for (int i = 0; i < K; i++) {
				...
				int[] rotationInfo = new int[5];  // 자석 번호를 1번 부터 사용
	...
	private static void effectRight(int magnetNo, int direction, int[] rotationInfo) {
		...
		// 기저부분 (4번 자석의 오른쪽에는 더이상 자석이 없으므로 종료)
		if (magnetNo == 4) {
			return;
		}
	...
	private static void effectLeft(int magnetNo, int direction, int[] rotationInfo) {
		...
		// 기저부분 (1번 자석의 왼쪽에는 더이상 자석이 없으므로 종료)
		if (magnetNo == 1) {
			return;
		}

		// 내 자석의 6번 극성과 왼쪽 자석의 2번의 극성이 다르면 회전
		// rotationInfo에 회전 정보를 저장
		if (magnets[magnetNo][6] != magnets[magnetNo - 1][2]) {
			rotationInfo[magnetNo - 1] = direction * -1;  // -1은 반대 방향 회전

			// 재귀 호출하여 다음 왼쪽 자석으로 이동
